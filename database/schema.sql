...
CREATE INDEX IX_AuditLog_table     ON Audit_Log (table_name);
CREATE INDEX IX_AuditLog_operation ON Audit_Log (operation);
CREATE INDEX IX_AuditLog_date      ON Audit_Log (changed_at);
GO
...
BEGIN
    ...
    /* Zero-line-item bills must stay pending — otherwise @paid >= @total is true when both are 0 (looks "already paid"). */
...
-- ── Patient CRUD Stored Procedures (missing from original schema) ────────
...
BEGIN
    ...
    SET first_name = @first_name, last_name = @last_name, phone = @phone,
        email = @email, address = @address, emergency_contact = @emergency_contact,
...
IF OBJECT_ID('dbo.usp_UpdatePatientFull', 'P') IS NOT NULL
    DROP PROCEDURE dbo.usp_UpdatePatientFull;
GO
CREATE PROCEDURE dbo.usp_UpdatePatientFull
    @patient_id INT,
    @first_name NVARCHAR(50),
    @last_name NVARCHAR(50),
    @dob DATE,
...
IF OBJECT_ID('dbo.usp_DeletePatient', 'P') IS NOT NULL
...
IF OBJECT_ID('dbo.usp_SearchPatientsCount', 'P') IS NOT NULL
    DROP PROCEDURE dbo.usp_SearchPatientsCount;
GO
CREATE PROCEDURE dbo.usp_SearchPatientsCount
    @search NVARCHAR(100) = NULL
AS
BEGIN
    SET NOCOUNT ON;
...
IF OBJECT_ID('dbo.usp_SearchPatients', 'P') IS NOT NULL
...
IF OBJECT_ID('dbo.usp_UpdatePatientProfile', 'P') IS NOT NULL
    DROP PROCEDURE dbo.usp_UpdatePatientProfile;
GO
CREATE PROCEDURE dbo.usp_UpdatePatientProfile
    @patient_id INT,
    @user_id INT,
    @email NVARCHAR(100) = NULL,
    @phone NVARCHAR(20),
...
IF OBJECT_ID('dbo.usp_GetPatientDashboardStats', 'P') IS NOT NULL
    DROP PROCEDURE dbo.usp_GetPatientDashboardStats;
GO
CREATE PROCEDURE dbo.usp_GetPatientDashboardStats
    @patient_id INT
AS
BEGIN
    SET NOCOUNT ON;
