-- ============================================================
-- MediCore HMS - Seed Data (PostgreSQL)
-- Run AFTER schema_postgres.sql
-- Password hashes are bcrypt hashes — see credentials at bottom.
-- ============================================================
...
(4, 'Omar',    'Siddiqui', 'Pediatrician',      '0302-3456789', 'omar@medicore.com',   2000.00, TRUE);

-- ── Nurses ────────────────────────────────────────────────────
INSERT INTO nurses (user_id, first_name, last_name, phone, email, assigned_ward) VALUES
...
INSERT INTO doctor_schedules (doctor_id, day_of_week, start_time, end_time, max_appointments) VALUES
(1, 0, '09:00', '17:00', 12),
(1, 1, '09:00', '17:00', 12),
(1, 2, '09:00', '17:00', 12),
(1, 3, '09:00', '17:00', 12),
...
-- ── Patients ──────────────────────────────────────────────────
INSERT INTO patients (user_id, first_name, last_name, dob, gender, phone, email, address, emergency_contact, blood_group, allergies) VALUES
...
('Amoxicillin 500mg',   'Antibiotic',    'Abbott',         45.00,    8, 20,  '2025-08-31'),
...
('Cetirizine 10mg',     'Antihistamine', 'ICI Pakistan',   18.00,    5, 20,  '2025-10-31'),
...
('Vitamin D3 1000IU',   'Supplement',    'Abbott',         60.00,    0, 30,  '2026-05-31'),
...
(1, 1, CURRENT_DATE - 7,  '10:00', 'completed', 'General checkup',       'Patient reports fatigue. BP normal. Advised rest.'),
(2, 2, CURRENT_DATE - 3,  '11:00', 'completed', 'Chest pain follow-up',  'ECG normal. Continue current medications.'),
(3, 1, CURRENT_DATE - 1,  '14:00', 'completed', 'Diabetes management',   'HbA1c slightly elevated. Increased Metformin dose.'),
(4, 3, CURRENT_DATE,      '09:00', 'scheduled', 'Child vaccination',      NULL),
(5, 2, CURRENT_DATE,      '11:00', 'scheduled', 'Cardiac consultation',   NULL),
(1, 1, CURRENT_DATE + 2,  '10:30', 'scheduled', 'Follow-up blood test',   NULL),
(2, 1, CURRENT_DATE + 5,  '09:00', 'scheduled', 'Routine checkup',        NULL),
(3, 3, CURRENT_DATE - 10, '10:00', 'cancelled', 'Appointment for flu',    NULL);

-- ── Bills ─────────────────────────────────────────────────────
INSERT INTO billing (patient_id, appointment_id, total_amount, paid_amount, status, payment_method) VALUES
...
-- ── Prescriptions ─────────────────────────────────────────────
INSERT INTO prescriptions (patient_id, doctor_id, appointment_id, notes, is_dispensed) VALUES
(1, 1, 1, 'Take with food. Avoid alcohol.',     TRUE),
(2, 2, 2, 'Continue current cardiac regimen.',  TRUE),
(3, 1, 3, 'Increase Metformin to twice daily.', FALSE);
...
(1, 6,  '400mg', 'Thrice daily',  '3 days',    9),
(2, 4,  '20mg',  'Once daily',    '30 days',  30),
(2, 8,  '10mg',  'Once daily',    '30 days',  30),
(3, 3,  '500mg', 'Twice daily',   '30 days',  60),
(3, 5,  '20mg',  'Once at night', '30 days',  30);

-- Reset sequences to avoid PK conflicts after explicit inserts
...
SELECT setval('nurses_nurse_id_seq',       (SELECT MAX(nurse_id)        FROM nurses));
SELECT setval('appointments_appointment_id_seq', (SELECT MAX(appointment_id) FROM appointments));
SELECT setval('billing_bill_id_seq',       (SELECT MAX(bill_id)         FROM billing));
SELECT setval('bill_items_item_id_seq',    (SELECT MAX(item_id)         FROM bill_items));
...
SELECT setval('prescription_items_pres_item_id_seq', (SELECT MAX(pres_item_id) FROM prescription_items));
SELECT setval('doctor_schedules_schedule_id_seq', (SELECT MAX(schedule_id) FROM doctor_schedules));
